...
async function validateOrigin() {
	var expectedRpId = this.expectations.get("rpId");
	var clientDataOrigin = this.clientData.get("origin");

	var origin = checkOrigin(clientDataOrigin);

	// https://www.w3.org/TR/webauthn/#relying-party-identifier
	if (!("." + new URL(origin).hostname).endsWith("." + expectedRpId)) {
		throw new Error("clientData origin does not end with expected rpId");
	}
...
async function validateRpIdHash() {
	...
	var expectedRpId = this.expectations.get("rpId");
	var createdHash = new Uint8Array(crypto.createHash("sha256").update(expectedRpId).digest()).buffer;
